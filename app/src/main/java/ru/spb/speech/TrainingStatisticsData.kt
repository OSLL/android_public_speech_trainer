...
class TrainingStatisticsData (myContext: Context, presentationData: PresentationData?, trainingData: TrainingData?) {
        ...
        get() {
            return if(trainingList != null && trainingCount != null){
                (DateUtils.formatDateTime(
                        context, trainingList[trainingCount-1].timeStampInSec!! * context.resources!!.getInteger(R.integer.milliseconds_in_second), DateUtils.FORMAT_SHOW_DATE) + " | " +
                        DateUtils.formatDateTime(
    ...
    val curWordCount: Int
        get() {
            return if (trainingList != null && trainingCount != null) {
                val tempWords = trainingList[trainingCount-1].allRecognizedText.split(" ")
                var curWordCount = 0
